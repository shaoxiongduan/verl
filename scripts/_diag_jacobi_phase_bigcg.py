"""Same as _diag_jacobi_phase.py but forces cudagraph capture sizes including
K+1=33 (the Jacobi num_tokens) so we can test the 'no captured graph' hypothesis.
"""
...
def main():
    ...
    # Force capture sizes to include K+1
    ...
    t0 = time.time()
    llm = LLM(**kw)
    print(f"[diag] mode={mode} cap={cap} load_s={time.time()-t0:.1f}", flush=True)
