#!/usr/bin/env bash
# On-policy cons-RL: Jacobi spec-decode rollout + cons loss reads cascade-evolved
# drafts from JACOBI_TRAJ_PATH (per-iter trajectories from jacobi_vllm_plugin).
#
# Per the audit, the on-policy noise distribution is the model's OWN cascade
# drafts at the SAME response positions as the cons-loss clean target. This
# fixes the OOD drift between training (uniform-random noise) and inference
# (collapsed "the-soup" cascade noise).
#
# Requires JACOBI_K == CONSISTENCY_BLOCK_SIZE so trajectories align to pack
# blocks. Default: both = 32.
set -xeuo pipefail

cd /mnt/weka/home/hao.zhang/shao/verl
source .venv/bin/activate

project_name=${PROJECT_NAME:-jacobi_forcing_dapo_deepscaler_onpolicy}
exp_name=${EXP_NAME:-jf_math_7b_dapo_ds_4gpu_jacobi_onpolicy_k32}

# === Jacobi spec-decode config ===
# K must match cons block_size for on-policy alignment.
export JACOBI_K=${JACOBI_K:-32}
# Per-step trajectory file so the cons hook only sees current-step rollouts.
# Set under the ckpt dir so trajectories live with the run artifacts.
CKPT_DIR=${CKPT_DIR:-/mnt/weka/home/hao.zhang/shao/verl/ckpts/${project_name}/${exp_name}}
mkdir -p "${CKPT_DIR}"
# Trajectory files now live in /dev/shm (RAM-backed): plugin writes per-request
# JSONL there; vllm_async_server reads + deletes per request. No persistent
# disk usage. Each file is tiny (~25KB) and short-lived (alive only while its
# request is in flight). Peak RAM use ~25-50 MB per worker.
...
# Cleanup any stale per-request files left from prior crashed runs (rare).
rm -f "${JACOBI_TRAJ_PATH}".req_*.jsonl 2>/dev/null || true

# === Cons loss config ===
export CONSISTENCY_ENABLE=${CONSISTENCY_ENABLE:-1}
export CONSISTENCY_ONPOLICY=${CONSISTENCY_ONPOLICY:-1}          # the new switch
# Filter cons-loss pool to rollouts with acc==1.0. Matches the proven-good
# ce_noisy_decay_k3 reference config — restricts cons supervision to
# trajectories the policy got right, so cons doesn't reinforce wrong drafts.
...
# block_size MUST equal JACOBI_K (validated by pack.py)
