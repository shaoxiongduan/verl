"""Phase profile vLLM Jacobi with cudagraph_mode=FULL (FIX E) on top of FIX A."""
...
def main():
    ...
    t0 = time.time()
    llm = LLM(**kw)
    print(f"[diag] mode=jacobi cg=FULL cap={cap} load_s={time.time()-t0:.1f}", flush=True)
