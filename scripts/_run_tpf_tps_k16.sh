#!/usr/bin/env bash
# K=16 block size bench: 6 ckpts x 2 BS x 2 T = 24 runs.
set -eu
...
K=16
MAXNEW=2048
OUT_DIR=eval_passk/tpf_results/k16
mkdir -p "$OUT_DIR"
...
run_one() {
  ...
  local traj=/tmp/vllm_k16_${tag}_${name}.jsonl
  rm -f "${traj}".*
  echo "=========================================="
  echo "=== K=16: $name | T=$T BS=$BS gmu=$GMU max_new=$MAXNEW ==="
  echo "=========================================="
  ...
  python3 scripts/_bench_postfix_tpf_tps.py \
    ...
    --max_new_tokens $MAXNEW --max_num_seqs $BS --temperature $T --gpu_mem_util $GMU 2>&1 | tail -10
}

# BS=1 phase (gmu=0.6)
for name in base_jf_math_7b math_ar_ds_step_300 dflashce_v2_step_60 dflashce_v2_step_300 dflashce_corrupt03_step_60 dflashce_corrupt03_step_300; do
  model=${MODELS[$name]}
  run_one "$name" "$model" 0.0 1  greedy_bs1 0.6
  run_one "$name" "$model" 1.0 1  onpol_bs1  0.6
done

# BS=16 phase (gmu=0.85)
for name in base_jf_math_7b math_ar_ds_step_300 dflashce_v2_step_60 dflashce_v2_step_300 dflashce_corrupt03_step_60 dflashce_corrupt03_step_300; do
  model=${MODELS[$name]}
