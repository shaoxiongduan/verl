"""Drive vllm_jacobi_patch_phase to collect per-section breakdown.

Run with VLLM_JACOBI_MODE=ar to baseline; VLLM_JACOBI_MODE=jacobi to profile spec.

Disables the auto-load plugin so we don't double-patch.
"""
from __future__ import annotations
import argparse, json, os, sys, time

# Prevent jacobi_vllm_plugin from monkey-patching out from under us.
os.environ["VLLM_PLUGINS"] = ""

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import vllm_jacobi_patch_phase as patch
patch.enable()
...
def main():
    ...
    mode = os.environ.get("VLLM_JACOBI_MODE", "jacobi")
    ...
    t0 = time.time()
    llm = LLM(**kw)
    print(f"[diag] mode={mode} load_s={time.time()-t0:.1f}", flush=True)
