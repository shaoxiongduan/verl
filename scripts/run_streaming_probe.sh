...
PROBE=/mnt/weka/home/hao.zhang/shao/verl/scripts/jf_streaming_probe.py
...
CUDA_VISIBLE_DEVICES=0 TPF_DATA_PATH=$DATA NUM_PROMPTS=$NUM N_TOKEN_SEQ_LEN=32 \
  ...
  LABEL=base DRAFT_INIT=prompt_sample OUT_JSON=/tmp/stream_base.json \
  $JF_PY $PROBE > /tmp/stream_base.log 2>&1 &
P1=$!
CUDA_VISIBLE_DEVICES=1 TPF_DATA_PATH=$DATA NUM_PROMPTS=$NUM N_TOKEN_SEQ_LEN=32 \
  ...
  LABEL=ar DRAFT_INIT=prompt_sample OUT_JSON=/tmp/stream_ar.json \
  $JF_PY $PROBE > /tmp/stream_ar.log 2>&1 &
P2=$!
CUDA_VISIBLE_DEVICES=2 TPF_DATA_PATH=$DATA NUM_PROMPTS=$NUM N_TOKEN_SEQ_LEN=32 \
  ...
  LABEL=t01 DRAFT_INIT=uniform OUT_JSON=/tmp/stream_t01.json \
  $JF_PY $PROBE > /tmp/stream_t01.log 2>&1 &
P3=$!
CUDA_VISIBLE_DEVICES=3 TPF_DATA_PATH=$DATA NUM_PROMPTS=$NUM N_TOKEN_SEQ_LEN=32 \
  ...
  LABEL=k3 DRAFT_INIT=uniform OUT_JSON=/tmp/stream_k3.json \
  $JF_PY $PROBE > /tmp/stream_k3.log 2>&1 &
P4=$!
wait $P1 $P2 $P3 $P4
echo STREAM_DONE
cp /tmp/stream_*.json /mnt/weka/home/hao.zhang/shao/verl/research/
echo COPY_DONE
